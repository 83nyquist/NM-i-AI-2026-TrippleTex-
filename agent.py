...
def run_agent(client: genai.Client, base_url: str, session_token: str, prompt: str, files: Optional[List[Any]] = None):
    ...
    12. INVOICING FLOW: CRITICAL: Before creating an invoice, you MUST verify the company has a bank account (see Rule 15). To create an invoice, you must FIRST create an order (`POST /order`) containing `orderLines` (which must include `product` and `count`). Never try to create an invoice directly via `POST /invoice` if the prompt asks to invoice a customer for a product. Also, when creating an order or invoice, ALWAYS fill in `orderDate` and `deliveryDate` (use current date like 2026-03-21 if not provided). Missing `deliveryDate` will cause a 422 error.
    ...
    19. POSTING ROWS & SUPPLIERS: When creating a manual voucher with a `postings` list, you MUST explicitly provide a `row` integer for each posting, starting from `1`. Never omit the `row` property and NEVER use row `0` (it is system generated). CRITICAL: NEVER put a `supplier` field on the root `Voucher` object! It MUST go inside a specific `Posting` row (e.g., account 2400). Do not include a `voucherType` unless explicitly known. Every manual voucher MUST have at least one posting.
    ...
    25. PROJECT ACTIVITIES: You MUST create project-specific activities via `POST /project/projectActivity`. When doing so, you must provide the `activity` as a nested object (e.g., `{"activity": {"id": ...}}`). Do NOT use `POST /activity` unless explicitly asked to create a generic activity type.
    ...
    34. EMPLOYMENT: When creating an employee (`POST /employee`), do NOT include `employmentDetails` in the payload. To add employment, you must first create the employee, and then use `POST /employee/employment`. On `POST /employee/employment`, you MUST provide a non-null `startDate` on the root object, and you MUST use `annualSalary` (NOT `yearlySalary`) inside the nested `employmentDetails` list.
    ...
    36. MANUAL VOUCHERS: When creating a manual voucher (`POST /ledger/voucher`), you MUST always provide a non-null `description` string on the root Voucher object. NEVER invent a `voucherType`, `voucherType.id`, `systemType`, `invoice`, or `dueDate` unless you have explicitly queried and verified it. Ensure you provide valid `postings` and NEVER use row 0.
